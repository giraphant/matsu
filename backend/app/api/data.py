...
# Webhook list endpoints for Charts page
@router.get("/webhooks")
async def get_all_webhooks(
    limit: int = Query(100, description="Number of records to return", ge=1, le=1000)
) -> list[Dict[str, Any]]:
    """
    Get all webhook data records for display in charts page.
    Returns the most recent records grouped by monitor_id.
    """
    db = get_db_session()
    ...
    try:
        ...
        # Get the latest record for each monitor_id
        subquery = db.query(
            MonitoringData.monitor_id,
            func.max(MonitoringData.timestamp).label('max_timestamp')
        ).group_by(MonitoringData.monitor_id).subquery()

        # Get full records for these latest entries
        latest_records = db.query(MonitoringData).join(
            subquery,
            (MonitoringData.monitor_id == subquery.c.monitor_id) &
        ...
        for record in latest_records:
            ...
            if time_diff.total_seconds() < 60:
                time_since = f"{int(time_diff.total_seconds())}s ago"
            elif time_diff.total_seconds() < 3600:
                time_since = f"{int(time_diff.total_seconds() / 60)}m ago"
            elif time_diff.total_seconds() < 86400:
                time_since = f"{int(time_diff.total_seconds() / 3600)}h ago"
            else:
                time_since = f"{int(time_diff.total_seconds() / 86400)}d ago"
            ...
            results.append({
                "id": str(record.id),
                "uid": record.monitor_id,
                "title": record.monitor_name or record.monitor_id,
                "text": record.text_value or record.status or "",
                "tags": tags,
                "data": data_dict,
...
async def get_webhook_history(
    uid: str = Query(..., description="Monitor ID"),
    limit: int = Query(100, description="Number of records to return", ge=1, le=1000)
) -> list[Dict[str, Any]]:
    """
    Get historical data for a specific webhook monitor.
    """
    db = get_db_session()

    try:
        records = db.query(MonitoringData).filter(
            MonitoringData.monitor_id == uid
        ...
        logger.error(f"Error getting webhook history: {e}", exc_info=True)
