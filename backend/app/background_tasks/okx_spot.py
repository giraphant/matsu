"""
OKX spot price monitor.
Fetches BTC, ETH, SOL spot prices from OKX and stores them in the database.
"""
...
class OKXSpotMonitor(BaseMonitor):
    ...
    async def run(self) -> None:
        ...
        logger.debug(f"Fetching OKX spot prices...")

        prices = await self._fetch_spot_prices()

        if not prices:
            logger.warning(f"No prices fetched from OKX")
            return

        stored_count = await self._store_prices(prices)
        logger.info(f"Stored {stored_count} OKX spot prices")

    async def _fetch_spot_prices(self) -> List[Dict[str, Any]]:
        """Fetch spot prices from OKX API."""
        try:
            ...
            logger.error(f"Error fetching OKX spot prices: {e}")
            return []
    ...
    async def _store_prices(self, prices: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            logger.error(f"Error storing OKX spot prices: {e}")
            db.rollback()
            return 0
